...
DRACO_ENC_LOSSLESS_OPTIONS = "-qp 0 -cl 0"  # default values

def draco_lossless(path_to_ply: str, output_folder_enc: str, output_folder_dec: str):
    ...
    print(f"Running draco lossless on {path_to_ply = }")
    ...
    @metrics
    def draco_enc():
        command = f"{DRACO_ENC} -i {path_to_ply} -o {enc_file} {DRACO_ENC_LOSSLESS_OPTIONS}"
        os.system(command)
