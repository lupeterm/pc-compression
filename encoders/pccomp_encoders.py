...
def octreezip(pfilename, pdepht):
    try:
        ...
        if len(ppoints) > 0:
            ...
            prec = np.amax(
                np.asarray([occ[2] - occ[1], occ[4] - occ[3], occ[6] - occ[5]])
                / (2**pdepht)
            )
            paramarr = np.asarray(
                [pdepht, occ[1], occ[2], occ[3], occ[4], occ[5], occ[6]]
            )  # depth and boundary
            np.savez_compressed(
                os.path.splitext(pfilename)[0] + "_ocz",
                points=occsorted,
                params=paramarr,
            )
            retmessage = (
                "Compressed into:"
                + str(os.path.splitext(pfilename)[0])
                + ".ocz | Storing resolution:"
                + str(prec)
            )
        else:
...
def zip_from_points(ppoints, pdepht, output_fn):
    try:
        if len(ppoints) > 0:
            occ = octreecodes(ppoints, pdepht)
            occsorted = np.sort(occ[0])
            ...
            np.savez_compressed(output_fn, points=occsorted, params=paramarr)
            retmessage = (
                "Compressed into:" + output_fn + " Storing resolution:" + str(prec)
            )
        else:
            retmessage = "PC is empty, bad, or missing"
    except Exception as e:
